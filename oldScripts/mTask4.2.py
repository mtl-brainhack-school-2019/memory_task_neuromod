# -*- coding: utf-8 -*-
"""
Created on Sun Sep  1 15:28:11 2019
...
def randStim(numTrials, nStim):
    trials = [[secrets.choice(categories[secrets.randbelow(len(categories))][secrets.randbelow(16)])for stim in range(nStim)]for trial in range(numTrials)]
    return (trials)
trials = randStim(2,8)
...
def randSign():#randomly generates 1 or -1 (quadrant position)
    if secrets.randbelow(2) == 0:
        ...
        return -1
#Working version, do not corrupt!!!
def encoding(categories, nStim, win):
    ...
    while stimCount <= nStim:
        for stim in range(nStim):
            stim = visual.ImageStim(win,image = randStim(nStim)[stim], color=(1,1,1), pos = (randSign()*250, randSign()*250), size = (500, 500))
            [stim.draw() for stim.image in randStim(nStim)]
            win.flip()
            core.wait(2.5)
            stimCount += 1
    win.close()
encoding1 = encoding(categories,8,win = visual.Window(size=(1000, 1000), color=(0, 0 , 0), units = 'pix')) 
...
def recall(categories, nStim, win):
    ...
    distractor = visual.ImageStim(win, random.choice(randStim()), color=(1,1,1), pos=(0.0,0.0), size=(500,500))
...
def runTask(numTrial, categories, nStim, win):
    #allTrials = [randStim(nStim) for trial in range(numTrials)]
    trialCount = 1
